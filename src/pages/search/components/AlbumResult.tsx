...
export function AlbumResults({ albums, query }: AlbumResultsProps) {
  if (albums.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-muted-foreground mt-1">We couldn't find any albums matching "{query}"</p>
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {albums.map((album) => (
        <div key={album.id} className="flex flex-col overflow-hidden rounded-lg border transition-all hover:shadow-md">
          ...
          <div className="flex flex-col p-4">
            ...
            <p className="text-sm text-muted-foreground">{album.user.fullName}</p>
          
            <div className="mt-2 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">{album.releaseDate}</span>
             
              <span className="text-xs text-muted-foreground">{album.songs.length} tracks</span>
            </div>
          </div>
        </div>
      ))}
